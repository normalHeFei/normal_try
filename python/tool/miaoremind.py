...
remind_msg = u'robot:小喵,吃药时间到,收到消息后要回复我[好的亲爱的],不然我会一直问的哦,哈哈哈'
...
def send_msg(msg):
    if not cache['username']:
        cache['username'] = itchat.search_friends(name=nick_name)[
            0]['UserName']
...
def remind():
    ...
    send_msg(remind_msg)
    print('到点,提醒吃药')
    scheduler.add_job(re_remind, 'interval',  seconds=15, id='re_remind_job')
